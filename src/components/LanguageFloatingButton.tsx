...
export function LanguageFloatingButton(): ReactNode {
	...
	return (
		<div className="group pointer-events-auto z-30 -mr-3 flex flex-col gap-2 p-3">
			<div className="flex flex-col gap-2 opacity-0 group-hover:opacity-100">
				{languages.map((lang) => (
					<button
						key={lang.code}
						className={clsx(
							'flex size-7 cursor-pointer items-center justify-center rounded active:scale-90',
							lang.colorClass
						)}
						type="button"
